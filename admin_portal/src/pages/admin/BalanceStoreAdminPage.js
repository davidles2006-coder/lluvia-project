// 这是 src/pages/admin/BalanceStoreAdminPage.js (100% 干净)
...
const UPLOAD_URL = `${API_BASE_URL}/admin/store/balance/upload/`; 

const BalanceStoreAdminPage = () => {
    ...
    const fetchItems = useCallback(async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${API_BASE_URL}/admin/store/balance/`, {
        ...
        setLoading(false);
    }, [staffToken,t]); // 🚩 修复：移除了不必要的 staffToken 依赖
    ...
    }, [staffToken, t]); // ⬅️ staffToken 在这里是必需的
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isEditing 
                ? `${API_BASE_URL}/admin/store/balance/${currentItemId}/`
                : `${API_BASE_URL}/admin/store/balance/`;
            ...
            const dataToSend = {
                ...
                balancePrice: parseFloat(formData.balancePrice || 0),
    ...
    const handleDeleteClick = async (itemId) => {
        if (window.confirm(t('admin.store.confirm_delete', 'Are you sure you want to delete this item?'))) {
            setLoading(true);
            try {
                await axios.delete(`${API_BASE_URL}/admin/store/balance/${itemId}/`, {
                    headers: { 'Authorization': `Token ${staffToken}` }
                });
                fetchItems(); 
            } catch (err) {
    ...
    return (
        <div className="admin-page-container">
            <h2>{t('sidebar.store_balance', 'Balance Store Management')}</h2>
            ...
            <div className="tab-content-real">
                ...
                <form onSubmit={handleSubmit} className="tab-form">
                    ...
                    <div className="form-group">
                        <label htmlFor="balancePrice">{t('admin.store.balancePrice', 'Balance Price ($)')}</label>
                        <input type="number" name="balancePrice" value={formData.balancePrice} onChange={handleInputChange} step="0.01" required />
                    </div>
                    <div className="form-group">
                ...
                <table className="admin-table">
                    <thead>
                        <tr>
                            ...
                            <th>{t('admin.store.balancePrice', 'Balance Price ($)')}</th>
                            <th>{t('admin.store.voucher_type', 'Linked Voucher Type (Product)')}</th>
...
export default BalanceStoreAdminPage;