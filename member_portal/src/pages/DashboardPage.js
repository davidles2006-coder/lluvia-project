...
function DashboardPage() {
  ...
  const handleRemoveAvatar = async () => { if (!window.confirm("确定?")) return; const token = localStorage.getItem('authToken'); try { await fetch(`${API_BASE_URL}/api/profile/avatar/`, { method: 'DELETE', headers: { 'Authorization': `Token ${token}` } }); setAvatarFile(null); setPreviewUrl(defaultAvatar); fetchAllData(); } catch (error) {} };
  ...
  if (isLoading || !userData) { return <div className="v11-dashboard-loading">{t('Loading Member Data...')}</div>; }
      ...
      {announcements.length > 0 && (
        <div style={{ marginTop: '40px', marginBottom: '30px' }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px', padding: '0 5px' }}>
             ...
             {announcements.length > 1 && <span style={{color: '#666', fontSize: '12px'}}> &gt;&gt; </span>}
           </div>
      ...
      {showModal && (
        <div className="v11-modal-overlay">
          <div className="v11-modal-content">
            <h3 className="v11-modal-title">{t('Edit Profile Details')}</h3>
            <form onSubmit={handleSaveProfile} className="v11-login-form">
              <div style={{textAlign: 'center', marginBottom: '20px'}}>
                ...
                {previewUrl !== defaultAvatar && (<span className="link-independent" onClick={handleRemoveAvatar} style={{cursor: 'pointer', fontSize: '14px', color: '#e74c3c'}}>{t('Remove Avatar')}</span>)}
              ...
              <div className="v11-input-group"><label style={{color: '#D4AF37'}}>{t('New Password (leave blank to keep)')}</label><input type="password" placeholder="******" value={editForm.password} onChange={(e) => setEditForm({...editForm, password: e.target.value})} /></div>
              <div className="v11-modal-actions"><button type="button" className="btn-cancel" onClick={() => setShowModal(false)}>{t('Cancel')}</button><button type="submit" className="btn-pill" disabled={isSaving}>{isSaving ? t('Updating...') : t('Save Changes')}</button></div>
            </form>
          </div>
        </div>
      )}
